...
export default function NameGenPage() {
  ...
  const generate = async () => {
    ...
    const prompt = `You are a creative branding expert for Indian startups. Generate exactly 5 unique, memorable business names for a ${industry} business${location ? ` in ${location}` : ""}. Tone: ${tone}.${keywords ? ` Keywords: ${keywords}.` : ""}\n\nFor each name provide: name (2-3 words, catchy), tagline (under 8 words), meaning (1 sentence why it works), domain (.com suggestion), logoIdea (1 sentence icon/style).\n\nRespond ONLY with a valid JSON array, no markdown, no code fences:\n[{"name":"...","tagline":"...","meaning":"...","domain":"...","logoIdea":"..."},...]`;
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,messages:[{role:"user",content:prompt}]})});
      const data = await res.json();
      const text = data.content?.find((b:any)=>b.type==="text")?.text||"";
      setResults(JSON.parse(text.replace(/```json|```/g,"").trim()));
    } catch { setError("Generation failed. Make sure the backend is running."); }
  ...
  return (
    <div style={{minHeight:"100vh",background:"var(--bg)",color:"var(--text)",fontFamily:"'DM Sans', sans-serif",padding:"36px 44px"}}>
      ...
      <div className="ph">
        ...
        <span className="ptag">✨ Claude AI</span>
      </div>
